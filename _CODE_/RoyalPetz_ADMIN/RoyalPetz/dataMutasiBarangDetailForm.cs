...
namespace AlphaSoft
{
    public partial class dataMutasiBarangDetailForm : Form
    {
        ...
        //private Data_Access DS_Branch = new Data_Access();
            ...
            switch (key)
            {
                ...
                case Keys.F2:
                    if (directMutasiBarang)
                    {
                        ...
                        { 
                            displayBarcodeForm = new barcodeForm(this, globalConstants.MUTASI_BARANG);
        ...
        public void addNewRowFromBarcode(string productID, string productName, int rowIndex = -1, int lotID = 0)
        {
            ...
            if (productLotID > 0 && gUtil.productExpirable(productID))
            {
                ...
                selectedRow.Cells["expiryDate"].Value = String.Format(culture, "{0:" + globalUtilities.CUSTOM_DATE_FORMAT + "}", Convert.ToDateTime(productExpiryDateValue));//globalUtilities.CUSTOM_DATE_FORMAT);
                ...
                //detailRequestOrderDataGridView.CurrentCell = selectedRow.Cells["expiryDateValue"];
            }
...
//                    clearUpSomeRowContents(selectedRow, rowSelectedIndex);
                ...
                while (rdr.Read())
                {
                    ...
                    else
                    {
                        ...
                        if (globalFeatureList.EXPIRY_MODULE == 1)
                        {
                            ...
                            {
                                if (gUtil.productExpirable(rdr.GetString("PRODUCT_ID")))
                                { 
                    ...
                    case globalConstants.NEW_PRODUCT_MUTATION:
                        ...
                        for (int i = 0; i < detailRequestOrderDataGridView.Rows.Count-1; i++)
                        {
                            ...
                            if (qtyApproved > 0)
                            {
                                ...
                                if (globalFeatureList.EXPIRY_MODULE == 1)
                                {
                                    if (gUtil.productExpirable(detailRequestOrderDataGridView.Rows[i].Cells["productID"].Value.ToString()))
                                    { 
                                        DateTime productExpiryDateValue = Convert.ToDateTime(detailRequestOrderDataGridView.Rows[i].Cells["expiryDateValue"].Value.ToString());
                                ...
                                if (globalFeatureList.EXPIRY_MODULE == 1)
                                {
                                    ...
                                    if (gUtil.productExpirable(expiryProductID))
                                    {
        ...
        private bool updateDataAtBranch(int approvedRO = 1)
        {
            ...
            if (branchID > 0)
            {
                ...
                {
                    // CONNECT TO BRANCH
                    gUtil.saveSystemDebugLog(globalConstants.MENU_MUTASI_BARANG, "TRY TO CONNECT TO BRANCH [" + branchIP + "]");
                    if (DS.Branch_mySQLConnect(branchIP))
                    {
                ...
                if (gUtil.matchRegEx(cellValue, globalUtilities.REGEX_NUMBER_WITH_2_DECIMAL))
                {
                    ...
                    try
                    {
                        ...
                        { 
                            if (globalFeatureList.EXPIRY_MODULE == 1)
                            {
                                if (gUtil.productExpirable(selectedRow.Cells["productID"].Value.ToString()))
                                {
