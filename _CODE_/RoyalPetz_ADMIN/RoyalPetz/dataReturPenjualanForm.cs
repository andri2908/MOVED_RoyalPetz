    ...
    public partial class dataReturPenjualanForm : Form
    {
        ...
        private string getPelangganName()
        {
            ...
            if (selectedCustomerID > 0)
                result = DS.getDataSingleValue("SELECT IFNULL(CUSTOMER_FULL_NAME, 'PELANGGAN UMUM') FROM MASTER_CUSTOMER WHERE CUSTOMER_ID = "+selectedCustomerID).ToString();
        ...
        private bool dataValidated()
        {
            ...
            if (globalFeatureList.EXPIRY_MODULE == 1)
            {
                ...
                for (i = 0; i < detailReturDataGridView.Rows.Count-1 && dataValid; i++)
                {
                    if (null != detailReturDataGridView.Rows[i].Cells["productID"].Value)
                    {
                        if (gutil.productExpirable(detailReturDataGridView.Rows[i].Cells["productID"].Value.ToString()))
                        { 
